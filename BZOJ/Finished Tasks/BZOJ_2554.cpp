...
int main() {
	...
	rep(i,n) ++cnt[s[i]-'A'];
	c[1]=c[0]=0;
	For(i,2,n) c[i]=2*c[i-1]-c[i-2]-(db)(n-1)/(n-i+1);
	f[1]=(db)-c[n]/n;
	For(i,2,n) f[i]=(db)2*f[i-1]-f[i-2]-(db)(n-1)/(n-i+1);
	For(i,1,n) f[i]=f[i]/i*n;
	rep(i,26) ans+=(db)cnt[i]/n*f[cnt[i]];
	//rep(i,n) printf("%.2f\n", f[i]);
	ans+=10, ans-=10;
